...
def validate_dag(
    *,
    ...
    The ``VERDICT_PASS`` rule is unconditional: such an edge *must* name the explicit
    Artifact hash it verifies. An opt-out was tried for live topologies that cannot know
    a digest before a Run publishes, but a Core constraint that a caller can switch off
    is not a constraint — it let a plan the Core must reject be submitted, and no
    harness-side check can restore that. A live planner instead revises the Plan once
    the artifact exists and pins the real hash.
    """
    if not nodes:
