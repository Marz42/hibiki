...
| 2026-09-13 | **Task G done**: real immutable ContextManifest (contract/deliverable/plan/dependency refs with version+hash, excluded categories, budget, measured mandatory bytes), `context_append` + `get_run_context` (run-bound, append-only, hashes recomputed from the Run's own workspace), the adapter prompt reads that context, and over-budget mandatory context blocks the Work Unit with `context_overflow` instead of truncating. 7 tests. |
| 2026-09-13 | **Task H done**: sandbox commands accept a cancel event and are killed immediately, so Cancel/Pause interrupt an in-flight `sleep 30` in a real container — adapter stop → exit 0.20s, Core `pause_task` → exit 0.18s (budget 15s), broker row recorded ALLOW/`cancelled`, no lingering container. |
| 2026-09-13 | **Task I done**: the three §24.4 recovery cases (model request interrupted → BLOCKED result, never success; executor vanishes → reconcile marks the Run LOST, emits `run.lost`, frees the Workspace and leaves the Work Unit retryable; crash during publication → orphan only, republishing succeeds and verifies) plus a conservative lost-run policy that never auto-retries a Run that may have produced an external effect. 5 tests. |
| 2026-09-13 | **Task K scaffolding done**: three fixed task contracts in `docs/m1/tasks/`, the `hibiki.interfaces.m1_runner` harness (records spec hash, manifest, tools, results, artifacts, timing per run; gates on verified artifacts and on any unbacked artifact reference), sandbox image `hibiki-sandbox:py312`, and a local OpenAI-compatible double for pre-credential validation. Harness smoke: **6/6** runs pass, including a real in-sandbox pytest run. |
| 2026-09-13 | Bugs the smoke caught and fixed: the adapter submitted a plain `fs.write` file digest as an artifact reference; the sandbox mounted a shared root instead of the Run's own workspace; broker-written files were unreadable by the sandbox uid. Full suite now **222 passed**. |
| 2026-09-13 | **H-022–H-032 acceptance tests added** (`tests/m1/test_h022_h032_acceptance.py`): simple Task straight to a worker with no Planner, FRESH context and no inheritance, mandatory-context overflow blocking, dynamic input appended with provenance, cross-Task/artifact refusal, traversal and symlink escape refused *and audited as DENY*, real sandbox with no network and no injected credentials, one writer per workspace, retry with a new run_id keeping the on-disk state, the publish crash window, and an adapter without a usable sandbox that never claims success. Broker fix: a path that escapes the workspace is now recorded as `DENY`/`path_escape` (and a missing workspace as `DENY`/`workspace_missing`) instead of an ALLOW row with a failed outcome. |
| 2026-09-13 | **M1 acceptance record published**: `docs/acceptance/M1-2026-09-13.md` with evidence in `docs/acceptance/evidence/m1-2026-09-13/`. Verdict **PENDING**: G1/G2/G4/G5/G6 met at `61bdc0d` (233 tests, hardened sandbox, stop 0.17s/0.21s, three recovery cases, complete per-run records), G3 partially met (6/6 against the provider double). |
| 2026-09-13 | **Adversarial review of M1** (independent pass): 4 P1 + 5 P2 found. Fixed and regression-tested in `tests/m1/test_m1_review_fixes.py`: stop now revokes the Run (was: RUNNING with a live credential), publish/append read through one descriptor (TOCTOU symlink escape closed), only content-backed hashes can be a verified delivery, sandbox stop reaps the process group, `record_model_usage` is Internal-only + run-bound + rejects negative counts, the Run wall clock is enforced, an empty completion is BLOCKED, reconcile applies the lost-run policy, and dependency sizes count towards the context budget. Full suite **245 passed**; record updated; pushed. |
| 2026-09-13 | **Live gate executed: 6/6 runs `COMPLETED`/`PASS`** (model `deepseek-flash`, 5.6–12.1 s and 4–7 model calls per run). Zero authorization violations: every `DENY` in the records is the model probing outside its grant and the Broker refusing it. Zero unbacked artifact references; every `verified_artifact_hash` matches held content; k3 ran pytest in the sandbox (`2 passed`, exit 0). Record verdict moved PENDING → **PASS**; raw runs in `docs/acceptance/evidence/m1-2026-09-13/live-runs/`. |
